class snake
{
    constructor(x, y, color)
    {
        ...
        canvas.style.border = x/32 + "px solid " + colorString(this.colorTone,0,0); //+ color;
        ...
        this.positionMatrix = []; //this matrix is going to hold where is snake's body to determine where we can create new food
        for(var i = 0; i < 9; i++)
        {
    ...
    addPart()
    {
        ...
        canvas.style.position = "absolute";
        canvas.style.border = this.x/32 + "px solid " + colorString(this.colorTone, 0, 0)// + this.color;
        canvas.style.backgroundColor = colorString(0, this.colorTone, 0);//color;
    ...
    move()
    {
        ...
        for(var i = this.canvasList.length-1; i > 0 ; i--) // switch positions to forward
        {
    ...
    createFood() // Gonna use another method for this later || New method to be implemented: Create food once in constructor then just move it when this function is called
    {
        ...
        do
        {
            w = randomInt(0, xAxis);
            h = randomInt(0, yAxis);
        }while(this.positionMatrix[h][w] == 1); //If snake's body is on the selected position, select another random position
        ...
        canvas.style.position = "absolute";
        canvas.style.border = this.x/32 + "px solid #009999"; //+ this.color;
